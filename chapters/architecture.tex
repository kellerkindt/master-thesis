...
\begin{itemize}
	...
	\item \label{message:extend} \textbf{EXTEND}: Extending an ongoing operation by pushing back its timeout.
...
The control of where the job is actually executed is to lost, which is not acceptable when different firewall rules apply to all Winslow instances.
When using GlusterFS, other cloud storage or compute capabilities, the storage is not always reachable through the same path for every node.
Nomad will only be used in the local Winslow container as thin layer to interfere with an enriched Docker API.
...
The first and second concern can be solved by providing a workspace directory for each stage, that is copied from the logically previous stage\footnote{the stage the new one is based on, this does not always need to be the \todo{numerical} previous stage}.
